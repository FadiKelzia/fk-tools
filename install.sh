...
# Simple Encrypted Installer - Password as Argument
# Usage: curl -s URL | bash -s -- PASSWORD
...
# Main execution
echo -e "${BLUE}Simple Encrypted Installer${NC}"
echo "=========================="
echo ""
...
if [ -z "$ACCESS_PASSWORD" ]; then
    echo -e "${RED}Usage: curl -s URL | bash -s -- 'PASSWORD'${NC}"
    echo -e "${YELLOW}Example: curl -s URL | bash -s -- 'your-access-password'${NC}"
    exit 1
fi
